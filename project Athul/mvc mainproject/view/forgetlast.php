<?php
session_start();
$id=isset($_POST['aeno']) ? $_POST['aeno'] : '';
//    ;
//echo $id;
//exit;
$name=$_SESSION['name'];
...
border: 1px #ffcf06 solid;">
    <input type="text" readonly class="form-control-plaintext" id="staticEmail2" value="NEW PASSWORD">
  </div>
           <marquee> <i><p>welcome back <?php echo $row['username'];?></p></i></marquee>
             <div class="col-auto">
    <p id="demoo">password</p>
    <input type="password" class="form-control" id="fpass" placeholder="password">
  </div>
  <div class="col-auto">
    <p id="demoo">confirm password</p>
    <input type="password" class="form-control" id="spass" placeholder="confirm password">
  </div>
  <div class="col-auto">
         <a href="forgetone.php"> <button type="button" class="btn btn-primary mb-3">back</button></a>

    <button type="button" class="btn btn-danger mb-3" onclick="validpass()">Change password</button>
   

  </div>
</form>
            <?php } else {?> <h5> sorry you entered details are not in our data base!!! please  try another </h5> 
            <a href="forgetone.php"> <button type="button" class="btn btn-primary mb-3">back</button></a>
        ...
        <footer class="bg-light text-center text-lg-start">
          ...
          <div class="text-center p-3 col-md-12" style="background-color: #234766 !important;">
            since@2020:
            <a class="text-dark" href="#">orientedweb</a><p id="value" value="<?php echo $row['id'];?>" style="visibility: hidden;"><?php echo $row['id'];?></p>
          </div>
          <!-- Copyright -->
        </footer>
